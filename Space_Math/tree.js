...
class TreeNode {
  ...
  combine(){
      ...
      if (this.isLeaf){
          ...
          if ((key == " " || key == "") && this.children.length > 1 && this.children[1].value == key){
              if (key == " "){
                  key = "\\,";
              }
              ...
              for (let i = 0; i < this.children.length; i++){
                  ...
                  if (newValue.includes("#@"+(i+1))){ // the rules used by super & subscripts
                      let expos = ["^","_","^^","__"];
                      let exemptionOnBracket = false;
                      for (let s of expos){
                          if (s == key){continue;}
                          if (childValueBracket.includes(s)){
                              exemptionOnBracket = true;
                              break;
                          }
                      }
                      if (childValueBracket.length > 1 && !exemptionOnBracket){
